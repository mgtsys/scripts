...
function check_varnish() {
    ...
    TEST_VARNISH=$(curl -u test:'!test123!' -i -H 'Accept:application/json' --resolve ${REDIRECTION_HTTPS}:443:${PRIVATE_IP} https://${REDIRECTION_HTTPS}:443 -Ik -s | sed -e 's/\(.*\)/\L\1/' | sed -n "/location/p" )
    if [[ ! -z ${TEST_VARNISH} ]]; then 
        REDIRECTION_HTTPS=$(echo ${TEST_VARNISH} | cut -d "/" -f 3)
    fi 
    CHECKOUT=$(curl -u test:'!test123!' -i -H 'Accept:application/json' --resolve ${REDIRECTION_HTTPS}:443:${PRIVATE_IP} https://${REDIRECTION_HTTPS}/checkout/:443 -Ik -s)
    VARNISH=$(curl -u test:'!test123!' -i -H 'Accept:application/json' --resolve ${REDIRECTION_HTTPS}:443:${PRIVATE_IP} https://${REDIRECTION_HTTPS}:443 -Ik -s)
    sleep 2
    VARNISH=$(curl -u test:'!test123!' -i -H 'Accept:application/json' --resolve ${REDIRECTION_HTTPS}:443:${PRIVATE_IP} https://${REDIRECTION_HTTPS}:443 -Ik -s | sed -e 's/\(.*\)/\L\1/' | sed -n "/x-cache-age/p" | cut -d " " -f 2)
