...
if [[ -z "${PLAN}" ]]; then
    echo "Please input a plan '-p basic | premium | ultimate | enterprise' "
    exit 1
...
if [[ ${ENV} == "live" || ${ENV} == "prod" ]]; then ENV=1
else ENV=0; fi
...
# checks if the ports in the nginx config file are ok and if the ownership of the files corresponds with that port number. 
    ...
    if [ -z "${PORTS}" ]; then
        ...
        for i in "${!PORTS[@]}"; do
            if [[ "${PORTS[$i]}" = "${NGINX_PORT}" ]]; then
                OWNER_FOLDER=$(stat -c "%U" /$APP_ETC/app/)
                if [[ "${OWNER_FOLDER}" != "${USERS[$i]}" ]]; then
                    ...
                    if [[ ${SET_PERM} == false ]]; then 
                        if [[ ${V} -eq 1 ]]; then MESS_PHP+="${OK}$File ${FILE}\n"; fi
    ...
    while read -r LINE; do
        ...
        if [[ $PREV_HOST != *"${SERVER_NAME}"* ]]; then
            ...
            if [[ "${PROTOCOL}" != " https" ]]; then 
                ...
                if [[ ${V} -eq 1 ]]; then MESS_NGINX+="${OK}HTTPS redirection ${SERVER_NAME}\n"; fi
            fi
        fi
    ...
    if [[ -z ${MYSQL_U} || -z ${MYSQL_P} || -z ${MYSQL_DB} ]]; then
        ...
        if [[ ${HASDB} = false ]]; then
            ...
            if [[ ${MYSQL_CONNECT} == "*ERROR*" ]]; then 
                ...
                if [[ ${V} -eq 1 ]]; then MESS_MYSQL+="${OK} DB = ${MYSQL_DB}\n"; fi
            fi
...
function check_basic_auth_alias_test() {
    ...
    if [[ "${BASIC_AUTH}" != *"restricted"* ]]; then
        ...
        if [[ ${V} -eq 1 ]]; then MESS_NGINX+="${OK}BASIC AUTH ${REDIRECTION_HTTPS}\n"; fi
...
function check_ssl_ww1_test_cert() {
    ...
    if [[ ${SSL_CERT} == *expired* ]]; then 
        ...
        if [[ ${V} -eq 1 ]]; then MESS_SSL+="${OK}${REDIRECTION_HTTPS}\n"; fi
    fi
}
            ...
            if [[ ${#SERVER_NAMES[@]} == 2 && ( ${BASE_URL} == *${SERVER_NAMES[0]}* || ${BASE_URL} == *${SERVER_NAMES[1]}* ) ]] || [[ ${BASE_URL} == *${SERVER_NAME}* ]]; then 
                ...
                if [[ ${#SERVER_NAMES[@]} == 2 ]]; then 
                    ...
                    # then normally if pre1 contains pre2 -> pre1 is an alias (www www1)
                        if [[ ${V} -eq 1 ]]; then MESS_NGINX+="${OK}${BASE_URL} - Alias ${ALIAS}\n"; fi
                    else 
        ...
        while read -r BASE_URL; do
            if [[ ${BASE_URL} == *${SERVER_NAME}* ]]; then
                ...
                if [[ ! (-z "${ALIAS}" || "${ALIAS}" == "www") ]]; then 
                    ...
                    if [[ ${V} -eq 1 ]]; then MESS_NGINX+="${OK}${BASE_URL} - Live domain"; fi
                fi
            fi
...
function check_admin_user() {
    ...
    if [[ ${HASDB} = false ]]; then
        ...
        if [[ -z ${USER} ]]; then 
            ...
            if [[ ${V} -eq 1 ]]; then MESS_MAG_CONFIG+="${OK}${REDIRECTION_HTTPS} - DB:${MYSQL_DB} - User admin for backend\n"; fi
        fi
        DBS+=(${MYSQL_DB})
    fi
}
...
function check_varnish() {
    ...
    #    we should check thath the MGT extension of varnish is there --> if it is not working only.  
        ...
        if [[ ${V} -eq 1 ]]; then MESS_VARNISH+="${OK}${REDIRECTION_HTTPS}\n"; fi
    fi
}
...
function check_redis() {
    if [[ ${HASDB} = false ]]; then
        ...
        if [[ ${DB_SESSION} && ${HAS_DB_CACHE} ]]; then 
            ...
            if [[ ${HAS_DB_SESSION} == true && ${HAS_DB_CACHE} == true ]]; then 
                if [[ ${V} -eq 1 ]]; then MESS_REDIS+="${OK}${CONFIG_FILE}\n"; fi
            fi
...
function check_cache_enabled() {
    ...
    if [[ "${ALL_CACHE_ENABLED}" = true ]]; then 
        if [[ ${V} -eq 1 ]]; then MESS_MAG_CONFIG+="${OK}CACHES ${REDIRECTION_HTTPS} enabled\n"; fi
    fi
}
