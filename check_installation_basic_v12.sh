...
function check_pub_folder() {
    ...
    if [[ "${CONFIG_FILE}" = *"env.php"* ]]; then
        ...
        if [[ "${MEDIA_BLOCKS}" == "0" ]]; then 
            MESS_NGINX+="     \e[38;5;198mWARNING: ${FILE} - You should have 'location /media/' block for 80 and 443 - check if images are loading\n"
        fi
...
function check_admin_user() {
    ...
    if [[ -z ${USER} ]]; then MESS_MAG_CONFIG+="${NOT_OK}${REDIRECTION_HTTPS} - There isn't any "admin" user for the backend\n"
    else MESS_MAG_CONFIG+="${OK}${REDIRECTION_HTTPS} - User "admin" for backend\n"; fi
}
...
function check_varnish() {
    ...
    if [[ $IS_BASIC_AUTH == true ]]; then
        ...
        else MESS_NGINX+="         \e[41mAn error has occurred: cannot restart nginx. Check 'nginx -t'\e[0m\n"; fi
    fi
    sleep 1
