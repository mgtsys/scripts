...
function check_admin_user() {
    ...
    if [[ -z ${USER} ]]; then
        MESS_MAG_CONFIG+="${NOT_OK}${REDIRECTION_HTTPS} - There isn't any "admin" user for the backend\n"
...
function check_mgt_user() {
    ...
    if [[ -z ${USER} ]]; then
        MESS_MAG_CONFIG+="${NOT_OK}${REDIRECTION_HTTPS} - There isn't any "mgtcommerce" user for the backend\n"
    fi    
}
