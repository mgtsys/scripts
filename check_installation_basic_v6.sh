...
function check_varnish() {
    ...
    curl -s -Ik https://"${REDIRECTION_HTTPS}" --resolve ${REDIRECTION_HTTPS}:${PRIVATE_IP}
...
function check_cache_enabled() {
    ...
    if [[ "${MAGENTO_VERSION}" = 2 ]]; then
        ...
        done < <(bin/magento cache:status | cut -d ": "-f 2) 
    else 
        ...
        while read -r l; do
            if [[ ${l} = 0 ]]; then echo -e "${NOT_OK}Cache Type ${CACHE_TYPES[$i]} is set to 0"; fi
            j=$((i+1))
